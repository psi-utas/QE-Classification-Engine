...
with st.sidebar:
    ...
    st.caption(
        "Your API key is used only for requests you submit during this session."
    )
...
st.success(
    "🔒 Privacy: No data stored. Files are processed in memory only and discarded immediately after results are returned."
)
...
search_text = st.text_input(
    "",
    placeholder="Type a payment description (e.g. Annual Leave, Casual Loading, Overtime, Redundancy Payment...)"
)
        ...
        if st.button(
            ...
            try:
                ...
                with st.spinner(
                    ...
                    st.session_state["bulk_result"] = classify_bulk(
                        input_df
                    )
